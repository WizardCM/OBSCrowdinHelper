...
/**
 * A class that takes over the login process to <a href="https://crowdin.com/">Crowdin</a> which is
 * required for mostly all of the {@link CrowdinRequest}s.
 * 
 * @author Vainock
 */
public class CrowdinLogin {
  ...
  /**
   * Logins to an account to contain specific cookies needed for mostly all of the
   ...
   * @param tfaCode - A {@link String} representing the Two-Factor Authentication code. If TFA is
   *        disabled, pass either an empty {@link String} or <code>null</code>.
   */
  public static void login(String login, String password, String tfaCode) {
    if (login.isEmpty() || login == null || password.isEmpty() || password == null)
      return;
    CrowdinRequest req = new CrowdinRequest().setUrl("accounts.crowdin.com/login")
        .setMethod(CrowdinRequestMethod.POST).setFormEntry("email_or_login", login)
  ...
  public static boolean loginSuccessful() {
    return !new CrowdinRequest().setUrl("crowdin.com/profile").setMethod(CrowdinRequestMethod.GET)
        .send().isUrlDifferent();
  }
}
