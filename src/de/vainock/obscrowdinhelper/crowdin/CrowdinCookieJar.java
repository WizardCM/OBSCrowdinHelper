...
public class CrowdinCookieJar implements CookieJar {
  ...
  private CrowdinCookieJar() {
    ...
    cookies.add(new Cookie.Builder().name("csrf_token").value(sb.toString()).domain("crowdin.com")
        .path("/").build());
  }
  ...
  void clearCookiesFromLogin() {
    ...
    for (Cookie cookie : cookies)
      if (cookie.name().equals("cid") || cookie.name().equals("token")
          || cookie.name().equals("csrf_token"))
