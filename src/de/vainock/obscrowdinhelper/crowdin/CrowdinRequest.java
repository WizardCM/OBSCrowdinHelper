...
/**
 * A class which simplifies the process of sending https requests to
...
public class CrowdinRequest implements Runnable {
  ...
  private Map<String, String> headers = new HashMap<String, String>(),
      parameters = new HashMap<String, String>(), formEntries = new HashMap<String, String>();
  private String url;
  private CrowdinRequestMethod method;
  private boolean trigger;

  /**
   * An object which simplifies the process of sending https requests to
   * <a href="https://crowdin.com/">Crowdin</a>.
   * 
   * @author Vainock
   */
   ...
   * @return a {@link String} representing the value of the parameter, or <code>null</code> if it
   *         doesn't exist.
   */
  ...
  /**
   * Sets a parameter of the {@link CrowdinRequest} which is later being converted into a
   ...
   * 
   * @author Vainock
   * @return a {@link String} representing the url of the {@link CrowdinRequest} without
   *         <code>https://www.</code> at the beginning.<br>
   ...
   * 
   * @author Vainock
   * @param url - A {@link String} representing the url of the {@link CrowdinRequest} without
   *        <code>https://www.</code> at the beginning.<br>
   *        <b>Example: </b><code>accounts.crowdin.com/login</code>
   * @return the {@link CrowdinRequest} itself.
  ...
  /**
   * Returns the {@link CrowdinRequestMethod} which is going to be used for the
   * {@link CrowdinRequest}.
   * 
   * @author Vainock
   * @return the {@link CrowdinRequestMethod} which is going to be used for the
   *         {@link CrowdinRequest}, or <code>null</code> if it doesn't exist.
   */
   ...
   * 
   * @author Vainock
   * @param method - The {@link CrowdinRequestMethod} which is going to be used for the
   *        {@link CrowdinRequest}.
   * @return the {@link CrowdinRequest} itself.
   ...
   * @return a {@link String} representing the value of a form entry, or <code>null</code> if it
   *         doesn't exist.
   */
  ...
  /**
   * Sets a form entry for requests with {@link CrowdinRequestMethod#POST} which is later being
   * converted to full form data.
   * 
   * @author Vainock
   * @param name - A {@link String} representing the name of the form entry.
  ...
  /**
   * Returns the amount of parallel running {@link CrowdinRequest}s used by {@link #send(List)} and
   * {@link #sendWithTrigger()}.
   * 
   * @author Vainock
   * @return an <code>int</code> representing the amount of parallel running
   *         {@link CrowdinRequest}s.
   */
  ...
  /**
   * Sets the amount of parallel running {@link CrowdinRequest}s used by {@link #send(List)} and
   * {@link #sendWithTrigger()}.<br>
   * <br>
   ...
   * 
   * @author Vainock
   * @param maxRunningRequests - An <code>int</code> representing the amount of parallel running
   *        {@link CrowdinRequest}s. The default value is <code>25</code>.
  ...
  public static void setMaxRunningRequests(int maxRunningRequests) {
    if (maxRunningRequests < 1)
      throw new IllegalArgumentException(
          "The amount of parallel running requests must be at least 1. Specified: "
              + maxRunningRequests);
  ...
  /**
   * Sends the current {@link CrowdinRequest} using the amount of parallel {@link CrowdinRequest}s
   * defined with {@link #setMaxRunningRequests(int)} and when finished triggers
   * {@link CrowdinRequestFinishedEvent#requestFinishedEvent(CrowdinResponse)} if registered with
  ...
  /**
   * Sends all specified {@link CrowdinRequest}s using the amount of parallel
   * {@link CrowdinRequest}s defined with {@link #setMaxRunningRequests(int)} and waits for them to
   * finish.
   * 
   * @author Vainock
  ...
  public void run() {
    try {
      ...
      if (method.equals(CrowdinRequestMethod.POST)) {
        ...
        reqBuilder.get();
      if (httpClient == null)
        httpClient = new OkHttpClient().newBuilder().followRedirects(true)
            .cookieJar(CrowdinCookieJar.getInstance()).build();
      Response response = httpClient.newCall(reqBuilder.build()).execute();
      CrowdinResponse res = new CrowdinResponse().setContent(response.body().string())
          .setCrowdinRequest(this).setStatusCode(response.code());
  ...
  /**
   * Sets all internal variables to <code>null</code> to reduce memory usage. If the variables are
   * being needed again, they are getting re-created.
   * 
   * @author Vainock
   * @param waitForRequests - A <code>boolean</code> representing if the method should either wait
   *        for all running requests to finish or if it should cancel all requests.
   */
